...
Deno.serve(async (req) => {
  ...
  for (const member of celebrants) {
    ...
    const personal = `Happy Birthday, ${member.full_name || "dear member"}! 🎉🎂 The entire __DC_ORG_NAME__ celebrates you today. May this new year overflow with God's grace, joy and favour!`;
      ...
      try {
        const emailResponse = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            from: fromEmail,
            to: [member.email],
            subject: "🎉 Happy Birthday from __DC_ORG_NAME__ Drama!",
            html: `<div style="font-family:Arial,sans-serif;max-width:520px;margin:auto;text-align:center">
              <div style="background:__DC_PRIMARY_COLOR__;color:#fff;padding:24px;border-radius:12px 12px 0 0">
                <h2 style="margin:0">🎂 Happy Birthday!</h2></div>
              <div style="border:1px solid #e2e8f0;border-top:none;padding:24px;border-radius:0 0 12px 12px">
                <p>${escapeHtml(personal)}</p>
                <p style="color:#64748b;font-size:13px">— __DC_ORG_NAME__ · DramaConnect</p>
              </div></div>`,
          }),
        });
